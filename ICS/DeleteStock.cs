...
namespace ICS
{
    public partial class DeleteStock : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand com1 = new SqlCommand("select CSTOCK from STOCKTABLE where PRODID = '" + comboBox1.SelectedItem + "'", con);
            double c;
            c = Convert.ToDouble(com1.ExecuteScalar());
            if (c == 0)
            {
                SqlCommand com = new SqlCommand("Delete from STOCKTABLE where PRODID = '" + comboBox1.SelectedItem + "'", con);
                com.ExecuteNonQuery();
                MessageBox.Show("       Record deleted Successfully!", "Congratulations!");
                MessageBox.Show("       Make sure to delete the same from : Product Entry", "ATTENTION!");
            ...
            {
                MessageBox.Show("       Stock Exists", "ATTENTION!");
            }
