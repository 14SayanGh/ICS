...
namespace ICS
{
    public partial class particular_purchase2 : Form
    {
        public particular_purchase2()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlDataAdapter da = new SqlDataAdapter("select * from PURCHTABLE where PRODNAME = '" + textBox1.Text + "'", con);
                DataSet ds = new DataSet();
                da.Fill(ds, "a1");
                textBox1.Text = ds.Tables["a1"].Rows[0][3].ToString();
                textBox2.Text = ds.Tables["a1"].Rows[0][2].ToString();
                textBox3.Text = ds.Tables["a1"].Rows[0][0].ToString();
